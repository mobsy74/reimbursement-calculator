...
def take_input():
    ...
    calculate_reimbursement(low_cost_dates, high_cost_dates)

def run_tests():
    files = ['set1.json', 'set2.json', 'set3.json', 'set4.json']
    print('')
    ...
    for filename in files:
        with open(filename, "r") as test_file:
            ...
            for project in test_set:
                start_date = validate_date(project['start'])
                end_date = validate_date(project['end'])
                cost = validate_cost(project['cost'])
...
def calculate_reimbursement(low_cost_dates, high_cost_dates):
